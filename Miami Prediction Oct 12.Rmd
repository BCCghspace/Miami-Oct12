...
#  houses %>%
 ...
 #                 TOD == 1  ~ "TOD",
 #                 TOD == 0  ~ "Non TOD"))
  #Error: Problem with `mutate()` input `TOD.cat`.
#x Input `TOD.cat` can't be recycled to size 3503.
#i Input `TOD.cat` is `case_when(TOD == 1 ~ "TOD", TOD == 0 ~ "Non TOD")`.
#i Input `TOD.cat` must be size 3503 or 1, not 7006.
...
##error: > length(houses$TOD) [1] 7006 while all the others are 3503

drop <- c("x","TOD")
houses = houses[,!(names(houses) %in% drop)]

houses %>% 
  dplyr::select(SalePrice, Stories, Pool) %>%
  mutate(Stories = as.factor(Stories),
         Pool = as.factor(Pool)) %>%
  filter(SalePrice <= 1000000) %>%
  gather(Variable, Value, -SalePrice) %>% 
...
##error:attributes are not identical across measure variables;
#they will be droppedError in melt_dataframe(data, id_idx - 1L, gather_idx - 1L, as.character(key_var),  : 
...
#Mean error and APE 432255.1   0.3407089   443611.7 0.1643853
mean(miami.test$SalePrice.AbsError, na.rm = T)
mean(miami.test$SalePrice.APE, na.rm = T)


#not working below
#error: Error in weighted.mean.default(d, weights, na.rm = TRUE) : 'x' and 'w' must have the same length
...
plot(houses$SalePrice, houses$lagPrice) 
...
miami.test %>% 
  ggplot(aes(SalePrice.Error, SalePrice)) +
  geom_point() +
...
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(miami.training) %>% 
                                 dplyr::select(NAME.y, SalePrice, ActualSqFt, Zoning, Stories.cat, elemCatch, Bath.cat, Pool, medHHInc, Dock, Bed.cat, middleCatch, age, pctVacant, pctRenterOcc, monthhousingcost, Patio, Wall, foodEstablishments, metromoverStops, metrorailStops, parkArea, TOD, floodInsureType, schoolDist, distWater)) #I put in the variables of regression 3

miami.test.nhood <-
  miami.test %>%
  mutate(Regression = "Neighborhood Effects",
         SalePrice.Predict = predict(reg.nhood, miami.test),
         SalePrice.Error = SalePrice - SalePrice.Predict,        #Dont need these three lines for the final predictions
         SalePrice.AbsError = abs(SalePrice - SalePrice.Predict),
         SalePrice.APE = (abs(SalePrice - SalePrice.Predict)) / SalePrice)%>%
  filter(SalePrice < 5000000)


#############examine the accuracy of neighborhood model########

bothRegressions <- 
  rbind(
    dplyr::select(miami.test, starts_with("SalePrice"), Regression, NAME.x, NAME.y) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    dplyr::select(miami.test.nhood, starts_with("SalePrice"), Regression, NAME.x, NAME.y) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))   
...
st_drop_geometry(bothRegressions) %>%
  ...
  group_by(Regression, Variable) %>%
    ...
    kable()
# BC: the Neighborhoods effects APE is inf, not sure why

# BC: THESE TWO SETS OF MAP LOOK ALMOST THE SAME
bothRegressions %>%
...
bothRegressions %>%
  dplyr::select(SalePrice.Predict, SalePrice, Regression) %>%
    ggplot(aes(SalePrice.Predict, SalePrice)) +
  geom_point() +
  stat_smooth(aes(SalePrice.Predict, SalePrice), 
             method = "lm", se = FALSE, size = 1, colour="#25CB10") +
  stat_smooth(aes(SalePrice, SalePrice), 
...
##########map the hh effects#########
names(bothRegressions)[names(bothRegressions) == "NAME.y"] <- "NAME"
st_drop_geometry(bothRegressions) %>%
  ...
  left_join(nhoods) %>%
    ...
    ggplot() + 
      ...
      mapTheme()
#BC: THIS IS LOOKING WEIRD, IT HAS SOMETHING TO DO WITH THE inf EARLIER IN THE neighborhoood APE
...
grid.arrange(ncol = 2,
  ggplot() + geom_sf(data = na.omit(acs), aes(fill = raceContext)) +
    scale_fill_manual(values = c("#25CB10", "#FA7800"), name="Race Context") +
    labs(title = "Race Context") +
    mapTheme() + theme(legend.position="bottom"), 
  ggplot() + geom_sf(data = na.omit(acs), aes(fill = incomeContext)) +
    scale_fill_manual(values = c("#25CB10", "#FA7800"), name="Income Context") +
    labs(title = "Income Context") +
    mapTheme() + theme(legend.position="bottom"))

st_join(bothRegressions, acs) %>% 
...
miami.test.nhood <-       
  miami.test %>%
  mutate(SalePrice.Predict = predict(reg.nhood, miami.test,
         TeamName = ))
...
#results of your cross-validation tests #AD: not sure what this means
